...
class TestDocConsistency:
    ...
    def test_no_known_stale_counts(self, doc_path):
        ...
        # 已知的错误计数声明（中英文均覆盖）
        # (?<![0-9]) 前缀防止匹配多位数字的一部分（如 39 里的 9）
        stale_patterns = [
            r"(?<![0-9])10\s*(个)?\s*(专业)?\s*[Aa]gent",
            r"(?<![0-9])11\s*(个)?\s*[Aa]gent",
            r"(?<![0-9])9\s*个\s*专业代理",
            r"(?<![0-9])13\s*(个)?\s*(核心)?\s*(技能|[Ss]kill)",
            r"(?<![0-9])15\s*(个)?\s*(快捷)?\s*(命令|[Cc]ommand)",
            r"(?<![0-9])8\s*(个)?\s*(规则|文件|[Rr]ule)",
            r"(?<![0-9])8\s*Rules",
        ]
